...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    # 而不是向量output_node。但这是损失函数的常见情况。    
    node_to_output_grads_list[output_tensor] = [out_grad]

    # 给定我们正在采用梯度的输出节点，以逆拓扑顺序遍历图。
    reverse_topo_order:list[Tensor] = list(reversed(find_topo_sort([output_tensor])))

    ### BEGIN YOUR SOLUTION
    for i in reverse_topo_order:
        v_i_bar=sum_node_list(node_to_output_grads_list[i])
        i.grad=v_i_bar
        if i.op is not None:
            for (k,grad_part) in zip(i.inputs,i.op.gradient_as_tuple(i.grad,i)):
                if k not in node_to_output_grads_list:
                    node_to_output_grads_list[k]=[]
                node_to_output_grads_list[k].append(grad_part)
    
    ### END YOUR SOLUTION


def find_topo_sort(node_list: List[Value]) -> List[Value]:
    """给定一个节点列表，返回以它们结尾的节点的拓扑排序列表。

    一个简单的算法是在给定节点上进行后序 DFS 遍历，
    根据输入边缘向后移动。由于节点已添加到排序中
    由于后序 DFS 遍历了它的所有前驱后，我们得到一个拓扑
    排序。
    """
    ### BEGIN YOUR SOLUTION
    ans=[]
